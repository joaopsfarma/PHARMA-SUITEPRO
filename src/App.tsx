...
import { LayoutDashboard, BrainCircuit, FlaskConical, Github, BookOpen, Sparkles, ShieldAlert } from 'lucide-react';
...
export default function App() {
  const [activeView, setActiveView] = useState<'calculator' | 'pharma_ai' | 'manual' | 'risk_score'>('calculator');
    ...
    <div className="min-h-screen flex flex-col bg-[#F8FAFC] font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      <header className="bg-white/70 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="font-display font-black text-2xl uppercase tracking-tighter text-slate-900 leading-none">Pharma<span className="text-blue-600">Suite</span></h1>
          ...
          <nav className="hidden md:flex bg-slate-100/80 p-1.5 rounded-2xl border border-slate-200/60">
            <button 
              onClick={() => setActiveView('calculator')} 
              className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${activeView === 'calculator' ? 'bg-white text-blue-600 shadow-md shadow-slate-200/50' : 'text-slate-500 hover:text-slate-800 hover:bg-white/50'}`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${activeView === 'risk_score' ? 'bg-white text-blue-600 shadow-md shadow-slate-200/50' : 'text-slate-500 hover:text-slate-800 hover:bg-white/50'}`}
            >
              <ShieldAlert size={16} />
              Score de Risco
            </button>
            <button 
              ...
              className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${activeView === 'pharma_ai' ? 'bg-white text-blue-600 shadow-md shadow-slate-200/50' : 'text-slate-500 hover:text-slate-800 hover:bg-white/50'}`}
            >
            ...
            <button 
              ...
              className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-sm font-semibold transition-all duration-300 ${activeView === 'manual' ? 'bg-white text-blue-600 shadow-md shadow-slate-200/50' : 'text-slate-500 hover:text-slate-800 hover:bg-white/50'}`}
            >
              <BookOpen size={16} />
              Manual
            </button>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-sm">
        <nav className="bg-slate-900/90 backdrop-blur-md p-1.5 rounded-2xl border border-white/10 shadow-2xl flex">
          ...
          <button 
            onClick={() => setActiveView('manual')} 
            className={`flex-1 flex flex-col items-center gap-1 py-2 rounded-xl transition-all ${activeView === 'manual' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}
          >
            <BookOpen size={18} />
            <span className="text-[9px] font-bold uppercase">Manual</span>
          </button>
        </nav>
      </div>
